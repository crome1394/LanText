...
if [ ! -f "$unsigned_rel" ]; then
  ...
  printf 'If Gradle wrote app-release.apk, release signing is still enabled.\n' >&2
  exit 1
fi
cp "$unsigned_rel" "$first_dir/lt.apk"

printf 'Build 2/2…\n'
"$gradle_bin" --no-daemon --no-parallel --no-configuration-cache clean
"$gradle_bin" --no-daemon --no-parallel --no-configuration-cache :app:assembleRelease
cp "$unsigned_rel" "$second_dir/lt.apk"

first_hash=$(hash_file "$first_dir/lt.apk")
second_hash=$(hash_file "$second_dir/lt.apk")
