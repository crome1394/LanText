#!/bin/sh
# Verify a signed APK uses the expected LanText release certificate.
#
# Expected SHA-256 digest is read from scripts/expected-release-cert.sha256
# (lowercase hex, no colons). Override with LT_EXPECTED_CERT_SHA256.
#
# Usage:
...
if [ -n "${LT_EXPECTED_CERT_SHA256:-}" ]; then
  expected_sha256=$(printf '%s' "$LT_EXPECTED_CERT_SHA256" | tr '[:upper:]' '[:lower:]' | tr -d ':\n ')
else
...
if [ -z "$expected_sha256" ] || [ "$expected_sha256" = "REPLACE_AFTER_KEYSTORE_CREATED" ]; then
  ...
  printf 'Run create-release-keystore.sh, then pin the digest.\n' >&2
  exit 1
fi
